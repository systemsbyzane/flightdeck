...
semantic-parity-local:
	...
	$(PYTHON) -c 'import json, pathlib; d=json.loads(pathlib.Path("$(LOCAL_EVIDENCE)/semantic-parity.json").read_text()); assert d["claim"]["locally_testable_mandatory_surfaces_pass"] is True; assert d["claim"]["overall_1_to_1_functional_parity"] is False'
