...
module Flightdeck
  # Read-only projection of durable Hub task and Mission records. It never
  # reads Codex global recents or infers telemetry from prose.
  class OperationsSnapshot
    ...
    MAX_ALERTS = 100
    STATUSES = %w[queued working waiting approval_required blocked review_ready failed_validation cancelled reconcile_required].freeze
    ...
    def snapshot
      ...
      operations = mission_operations + task_operations
      ...
      alerts = operations.filter_map do |operation|
        ...
        { "operation_id" => operation.fetch("operation_id"), "status" => operation.fetch("status") }
    ...
    def runtime_capabilities!(compatibility)
      ...
      unless value["primary_runtime"] == "codex" && adapters.is_a?(Hash) &&
             ...
             controls.is_a?(Array) && controls.uniq == controls && controls.all? { |control| %w[model reasoning_effort].include?(control) }
    ...
    def mission_operations
      ...
      record_ids(@config.mission_dir, "mission").map do |id|
        ...
        operation(
          ...
          nodes, skills: skill_summary(events)
        )
      end
    end
    ...
    def task_operations
      ...
      record_ids(@config.task_dir, "task").map do |id|
        ...
        operation(
          "task", id, task.dig("metadata", "title"), task_status(task),
          task.dig("metadata", "created_at"), task.dig("metadata", "updated_at"),
          { "workload" => workload }, children,
          validation: task_validation(task), skills: unavailable_skills
        )
      end
    end

    def operation(kind, id, title, state, created_at, updated_at, route_scope, children, validation: nil, skills: unavailable_skills)
      value = {
        ...
        "route_scope" => route_scope.reject { |_key, item| item.nil? },
        "children" => children,
        "skills" => skills
      }
    ...
    def task_child(unit, workload)
      ...
      child(unit.fetch("id"), unit.fetch("logical_project_key"), unit.fetch("state"), nil, nil,
            workload: workload, skills: unavailable_skills)
    end

    def child(id, logical_key, state, created_at, updated_at, workload: nil, activity: nil, observed_at: nil, validation: nil, artifacts: nil, skills: unavailable_skills)
    ...
    def task_validation(task)
...

      {
        ...
        "unavailable" => checks.count { |check| %w[skipped unavailable].include?(check["status"]) }
      }
    end
