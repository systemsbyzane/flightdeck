  ...
  class OperationAuthoring
    ...
    def snapshot_detail_identities
      ...
      operation_records.each_with_object({}) do |record, identities|
        ...
        if state == "unresolved" && result["outcome"] == "unresolved" && result["mission_id"].nil? &&
           result["reason"] == "no_operation_persisted"
          next
        end
    ...
    def verify_capability!
      ...
      paths = [compatibility_path, authoring_lock_path] + schemas.map { |name| File.join(config.root, "hub", "schemas", name) }
      ...
      managed = ["hub/.operation-authoring.lock", "lib/flightdeck/operation_authoring.rb"] + schemas.map { |name| "hub/schemas/#{name}" }
    ...
    def operation_records
      ...
      mission_ids = records.map { |record| record.fetch("mission_id") }
      unless mission_ids.uniq.length == mission_ids.length
        raise ContractError.new("operation_store_invalid", "operation store contains conflicting Mission identities")
      end
    ...
    def validate_operation_record!(record)
      ...
      unless record["operation_digest"] == Digest::SHA256.hexdigest(record["operation_id"]) &&
             ...
             record["mission_id"].to_s.match?(/\Aoperation-[0-9a-f]{24}\z/) && record["operation_id"] == record["mission_id"] &&
             RECORD_STATES.include?(record["state"])
    ...
    def authoring_lock_path
      config.root_path("hub/.operation-authoring.lock", label: "Operation authoring lock")
    end

    def with_authoring_lock(mode)
      stat = File.lstat(authoring_lock_path)
      unless stat.file? && !stat.symlink?
        raise ContractError.new("capability_incomplete", "Operation authoring lock boundary is unavailable")
      end
      File.open(authoring_lock_path, File::RDONLY) do |lock|
