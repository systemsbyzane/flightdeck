...
4. On `created`, use the returned `mission_id` / `snapshot_operation_id` only
   to reconcile the existing read-only Operations snapshot. The snapshot keeps
   that `mission:` value as source identity and exposes canonical detail
   identity separately only after the persisted authoring record and Mission
   binding reconcile exactly. A replay of the byte-equivalent launch returns
   the same created result with `replayed:true`.
